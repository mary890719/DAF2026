(() => {
  ...
  const workCards = works => works.map(w => `<a class="work-card" href="work-detail.html?id=${w.id}"><div class="work-card-media">${C.placeholder(w.images?.[0]?.alt || "作品圖片","")}<div class="work-card-overlay"><span>${[w.creators?.map(creator => creator.name).join("／"), w.medium].filter(Boolean).join("<br>")}</span></div></div><div class="work-label"><span class="work-no">${w.number}</span>${w.title}</div></a>`).join("");
  ...
  const pages = {
    ...
    works: () => `<main class="container">${C.crumb("展覽資訊")}<h1 class="section-title">參展作品</h1><div class="works-grid works-grid-main">${workCards(D.works)}</div><section class="section"><h2 class="section-title">聲響藝術家</h2><div class="works-grid works-grid-sound">${workCards(D.soundArtists)}</div></section></main>`,
    ...
    timeline: () => `<main class="container">${C.crumb("活動時程")}<h1 class="page-title">活動時程</h1><div class="timeline">${D.events.map(event=>`<a class="event-row" href="event-detail.html?id=${event.id}"><span class="event-type">${event.type}</span><strong>${event.title}</strong><span>${event.date}・${event.time}</span></a>`).join("")}</div></main>`,
    "event-detail": () => { const event=D.events.find(item=>item.id===queryId()) || D.events[0], leader=event.speaker || event.instructor; return `<main class="container">${C.crumb([{label:"活動時程",href:"timeline.html"},{label:event.title}])}<article class="event-detail"><p class="event-detail-type">${event.type}</p><h1>${event.title}</h1><dl>${[["日期",event.date],["時間",event.time],["地點",event.location],[event.type === "講座" ? "講者" : "帶領者",leader]].filter(([,value])=>value).map(([label,value])=>`<dt>${label}</dt><dd>${value}</dd>`).join("")}</dl><section><h2>活動介紹</h2><p>${event.description}</p></section><section><h2>活動紀錄</h2><div class="event-gallery">${event.images.map(image=>C.placeholder(image.alt)).join("")}</div></section></article></main>`; },
  ...
  if (page === "home" && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    const glyphs = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789#@%&▓";
    const scramble = (element, delay = 0) => {
      const finalText = element.textContent, duration = 720, frame = 40;
      window.setTimeout(() => {
        const startedAt = performance.now();
        const tick = now => {
          const progress = Math.min((now - startedAt) / duration, 1);
          const resolved = Math.floor(progress * finalText.length);
          element.textContent = [...finalText].map((character, index) => character === " " || index < resolved ? character : glyphs[Math.floor(Math.random() * glyphs.length)]).join("");
    ...
    document.querySelectorAll(".hero-scramble").forEach((element, index) => scramble(element, index * 140));
  }
