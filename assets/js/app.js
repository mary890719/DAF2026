(() => {
  ...
  const workCards = works => works.map(w => `<a class="work-card" href="work-detail.html?id=${w.id}">${C.placeholder("","")}<div class="work-label"><span class="work-no">${w.number}</span>${w.title}</div></a>`).join("");
