...
window.DAF_DATA.shops.forEach(shop => {
  ...
  shop.images = Array.from({length: shop.imageCount || 0}, (_, index) => {
    const number = index + 1;
    const extension = shop.imageExtensionOverrides?.[number] || "jpg";
    return `${shop.imageFolder}/${String(number).padStart(2, "0")}.${extension}`;
  });
});
